---
layout: post
title:  "[SunFlower] 🛠 안드로이드 앱 빌드는 어떻게 실행되는 걸까?"
...
안드로이드 스튜디오는 새 프로젝트를 생성할 때 아래 그림과 같은 파일들을 자동으로 생성하고 기본값들로 파일들을 채우게 된다.
...
이를 위해 다음과 같이 최상위 build.gradle 파일에 __ext__ 블록을 만들고 그 안에 속성들을 정의하면 된다.
