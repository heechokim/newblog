...
다이나믹 프로그래밍이라는 용어에서 "다이나믹" 은 __"다단계적"__ , __"시가변적"__ 이라는 의믜를 가지고 있다.

> --> [다이나믹 프로그래밍의 역사](https://ko.wikipedia.org/wiki/%EB%8F%99%EC%A0%81_%EA%B3%84%ED%9A%8D%EB%B2%95) 에 대한 위키피디아 문서 참고!

다이나믹 프로그래밍은 __큰 문제를 작은 문제로 쪼개서 풀어 나가는 알고리즘__ 이다.
...
* __큰 문제를 작은 문제로 쪼개서 풀어 나가는 알고리즘은 두 가지 방법이 존재한다!__
	...
	* __다이나믹 프로그래밍__ : 큰 문제를 작은 문제로 쪼갰을 때 각 작은 문제들은 __중복이 가능하다!__

		예 ) 어떤 장소에 40명이 존재하는데 이를 10명과 30명으로 나눌 수 있다. 또 이 30명을 다시 10명과 20명으로 나눌 수 있고, 다시 이 20명을 10명과 10명으로 나눌 수 있을 것이다.

		즉, 작은 문제들은 모두 10명이 되었으므로 작은 문제들끼리 중복이 존재한다고 볼 수 있다.

		나중에 이렇게 중복되는 작은 문제들을 __점화식__ 이라고 부른다.

		따라서 작은 문제들이 중복되므로 이 작은 문제들을 모두 푸는 것이 아니라 한 번만 풀고 어딘가에 답을 저장해둔 후, 큰 문제를 풀 때 필요하면 저장된 값을 가져다가 사용한다. --> 효율성이 높다!
