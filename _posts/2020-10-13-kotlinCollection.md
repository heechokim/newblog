...
배열들을 공부하다 보니 Collection 패키지에 대해 공부하게 되었고, 한 번 정리를 해야겠다고 생각했다.

코틀린은 자체적으로 내장하고 있는 표준 라이브러리(kotlin standard library = kotlin stdlib) 를 가지고 있다.
...
[kotlin standart library 공식 문서](https://kotlinlang.org/api/latest/jvm/stdlib/) 에 보면 
...
위와 같이 라이브러리 안에 정말 많은 패키지들이 있고, 그 중 __kotlin.collections__ 라는 이름의 collection 패키지가 있는 것을 볼 수 있다.

그럼 이 패키지는 어떤 기능을 하는 것들을 묶어놓은 폴더인지 알아보자!
...
여기서 그룹이라는 것의 예로는 일반적인 1차원 배열, 2차원 배열, Map, Set 등의 자료구조들이 포함된다.

코틀린에서는 이러한 그룹들을 크게 __List__, __Map__, __Set__ 이라는  3가지 큰 분류로 나누어 collection 패키지 안에 넣어놓았다.

각각의 List, Map, Set은 또 더 다양한 자료구조로 나뉜다.
...
ArrayList가 구현되어 있는 모습을 보면 위 다이어그램에서 볼 수 있는 [MutableList](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/-mutable-list/) 라는 인터페이스를 구현하여 만들어져 있음을 알 수 있다.
